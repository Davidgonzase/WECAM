...
startButton.addEventListener("click", async () => {
    ...
    namestream = name
    if (!name) {
        errorBox.textContent = "Introduce un nombre de stream.";
        return;
    }

    errorBox.textContent = "";
    status.textContent = "Verificando nombre...";
    ...
    try {
        ...
        console.error(err);
        errorBox.textContent = "Error al contactar con la API.";
    }
});
...
async function startStreaming(name) {
    ...
    async function newcam() {
        try {
            console.log("jwt :" + jwt);
            const response = await fetch(`${apiUrl}/newcam`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
